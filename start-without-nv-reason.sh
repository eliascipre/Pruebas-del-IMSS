...
# 🚀 Script de Inicio Unificado - Suite IMSS (Sin NV-Reason-CXR)
# Este script levanta todos los servicios localmente excepto NV-Reason-CXR
# Útil cuando no hay suficiente memoria VRAM para ejecutar el modelo NV-Reason-CXR
...
show_status() {
    ...
    local services=("Gateway:3001" "Chatbot:5001" "Educacion:5002" "Simulacion:5003" "Radiografias:5004")
    ...
    echo ""
    print_warning "⚠️  NV-Reason-CXR está deshabilitado (no hay suficiente VRAM)"
    echo ""
}
...
cleanup() {
    ...
    for port in 3001 3000 5001 5002 5003 5004; do
        kill_port $port
    done
...
# Función principal
main() {
    echo "🏥 SUITE IMSS - INICIO UNIFICADO (SIN NV-REASON-CXR)"
    echo "===================================================="
    echo ""
    print_warning "⚠️  NV-Reason-CXR está deshabilitado por limitaciones de VRAM"
    echo ""
    ...
    print_warning "NV-Reason-CXR omitido (requiere VRAM adicional)"
    ...
    echo ""
    print_warning "⚠️  NV-Reason-CXR no está disponible (deshabilitado por VRAM)"
    echo ""
    ...
    while true; do
        ...
        for port in 3001 5001 5002 5003 5004; do
            if ! check_port $port; then
                all_active=false
                break
            fi
